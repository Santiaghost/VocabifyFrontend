...
import { encode } from "base-64";
  ...
  const EditExercisesPage = ({modalEditExercisePage,setModalEditExercisePage , userProperties, setUserProperties, selectedExercise, setSelectedExercise, tokenAPI} : any) =>{
    ...
    const [enunciado, setEnunciado] = useState(selectedExercise.titulo)
    const [idioma, setIdioma] = useState(selectedExercise.idioma)
    const [tipoEjercicio, setTipoEjercicio] = useState(selectedExercise.tipoejercicio)
    const [nivel, setNivel] = useState(selectedExercise.nivel)
    const [error, setError] = useState<string | null>(null);
    const [apartados, setApartados] = useState< Apartado[]>(
        selectedExercise.apartados
    );
    ...
    const [modalExercisesPageKey, setModalEditExercisePageKey] = useState(0)
    ...
    useEffect(() => {
        setEnunciado(selectedExercise.titulo)
        setIdioma(selectedExercise.idioma)
        setTipoEjercicio(selectedExercise.tipoejercicio)
        setNivel(selectedExercise.nivel)
        setApartados(selectedExercise.apartados)
    },[modalEditExercisePage])
          ...
          if (granted === PermissionsAndroid.RESULTS.GRANTED) {
              ImagePicker.openPicker({
                  ...
                  if (image.path) {
                    ...
                    apartadosAux[index].imagenCambiada = true
                    setApartados(apartadosAux)
                  }
                }).catch(error => {
                  console.log(error);
  ...
  const fotoApartadoHandler = (index : any) =>{
    ...
    const backHandler = () =>{
        setEnunciado(selectedExercise.enunciado)
        setIdioma(selectedExercise.idioma)
        setTipoEjercicio(selectedExercise.tipoEjercicio)
        setNivel(selectedExercise.nivel)
        setApartados(selectedExercise.apartados)
        ...
        setModalEditExercisePage(false)

        setModalEditExercisePageKey(modalExercisesPageKey + 1)
    ...
    const showAlert = () => {
      Alert.alert(
        ...
        'El ejercicio se modificó correctamente',
        [
          {
            text: 'Aceptar',
            onPress: undefined, 
    ...
    const editarEjercicioHandler = () =>{
      ...
      if(apartados.length < 1 ){
        ...
        if(error != null){
            ...
            var editExerciseEndpoint = config.API_APP + config.EXERCISE_PATH + config.EXERCISE_UPDATE_PATH
            const credencialesAPI = encode(`${tokenAPI}:`);
            ...
            axios.put(editExerciseEndpoint,data, configHeathers).then((response) =>{
                  if(response.status == 200){
                    var indice = 0
                    response.data.apartados.map((apartadoFoto: { imagenAdicional: string | undefined; id: any; }) => {
                      console.log(apartadoFoto.imagenAdicional)
                      console.log(apartados[indice].imagenAdicional)
                      if(apartados[indice].imagenAdicional != undefined && apartados[indice].imagenCambiada == true){
    ...
    return(
        <Modal visible = {modalEditExercisePage} animationType="slide" key={modalExercisesPageKey}>
                ...
                <Pressable onPress={backHandler} style={styles.backButton}>
                    ...
                    <View style ={styles.textos}>
                        <View style = {styles.exerciseView}>
                            <Text style = {styles.txtexercise}>Modificar Ejercicio</Text>
                        </View>
                    ...
                    <Pressable style = {styles.btnSubmit}
                        onPress={editarEjercicioHandler}
                        >
                            <Text style = {styles.btnSubmitText}>Editar Ejercicio</Text>
                        </Pressable>
